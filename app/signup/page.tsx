...
export default function SignupPage() {
  ...
  const handleGoogle = async () => {
    try {
      ...
      if (err?.response?.status === 401) {
        ...
        setErrorMessage(
          "We couldn't complete your F3 Invigorate account setup. This is a server-side authentication issue — please try again in a few minutes."
        );
        setLoading(false);
        return;
      }
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Failed to sign up. Please check your credentials.');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8 bg-white/5 backdrop-blur-sm rounded-2xl p-8 shadow-2xl border border-white/10">
        ...
        {!showEmailForm ? (
          <>
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 py-3 px-6 border-2 border-white/30 rounded-lg shadow-lg text-base font-medium text-white bg-white/10 hover:bg-white/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white/50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              {loading ? (
                <>
          ...
          <>
            ...
            <form onSubmit={authMode === 'signup' ? handleEmailSignup : handleEmailSignin} className="space-y-4">
              <input
                ...
                className="w-full px-4 py-3 bg-white/10 border border-white/30 rounded-xl text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-white/50"
                required
                disabled={loading}
              />
              <input
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-white text-black py-3 px-6 rounded-xl font-semibold hover:bg-white/90 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
